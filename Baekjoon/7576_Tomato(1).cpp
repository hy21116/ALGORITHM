...
>>>>>>> 91f7bfc950979894c912f4701f63450f35db0867
int queue[1000000];
int front, rear;
int countDay = 0;
...
<<<<<<< HEAD
	...
	while (queue_size()){
		...
		for (int i = 0; i < prevSize; i++){
			...
			if (box[cur_row - 1][cur_col] == 0 && cur_row - 1 >= 0){
				box[cur_row - 1][cur_col] = 1;
				queue_push((cur_row - 1) * box_W + cur_col);
			}
			if (box[cur_row][cur_col + 1] == 0 && cur_col + 1 < box_W){
				box[cur_row][cur_col + 1] = 1;
				queue_push((cur_row)* box_W + cur_col + 1);
			}
			if (box[cur_row + 1][cur_col] == 0 && cur_row + 1 < box_H){
				box[cur_row + 1][cur_col] = 1;
				queue_push((cur_row + 1) * box_W + cur_col);
			}
			if (box[cur_row][cur_col - 1] == 0 && cur_col - 1 >= 0){
				box[cur_row][cur_col - 1] = 1;
				queue_push((cur_row)* box_W + cur_col - 1);
			}
		}
